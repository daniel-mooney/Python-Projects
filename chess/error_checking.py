...
def check_valid_move(current_coord: str, move_coord: str, player_number: int, board):
    ...
    if not check_valid_movement(current_num, move_num, piece_type, target_square):
        print(f"Error: Cannot move {piece_type, current_coord} to {move_coord} square.")

    return True

def check_valid_movement(current_num: int, move_num: int, piece_type: str, target_square: str):
    
    diagonals = [9, 11]
    lateral_vertical = [1, 2, 3, 4, 5, 6, 7, 10]
    knight = [8, 12, 19, 21]
    king = [1, 9, 10, 11]
    pawn, pawn_taking = [10], [9, 11]

    peice_type = piece_type.lower()
    square_difference = abs(current_num - move_num)

    if piece_type == 'k' and square_difference not in king:
        return False
    elif piece_type == 'q' and square_difference not in diagonals + lateral_vertical:
        return False
    elif peice_type == 'r' and square_difference not in lateral_vertical:
        return False
    elif peice_type == 'n' and square_difference not in knight:
        return False
    elif peice_type == 'b' and square_difference not in diagonals:
        return False
    elif piece_type == 'p' and target_square == '0' and square_difference not in pawn:
        return False
    elif piece_type == 'p' and target_square != '0' and square_difference not in pawn_taking:
        return False
